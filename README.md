...
- **🕒 歷史紀錄本機儲存與匯出匯入**
  ...
  利用瀏覽器 IndexedDB 自動保存已辨識的名片紀錄；從歷史紀錄載入後，手動修改欄位亦可自動寫回儲存。另支援一鍵「匯出備份」與「匯入還原」JSON 格式檔案，方便跨裝置備份。

- **🎚️ 全域文字大小等比例縮放**

  內建全網頁級文字等比例縮放機制，提供 `90% (較小)`、`100% (標準)`、`110% (較大)`、`125% (超大)` 四種尺寸切換與即時點擊預覽，具備防閃爍同步加載與取消回滾機制。
...
- 2026-05-19：1.38 版。新增歷史紀錄的「匯出」與「匯入」功能。
- 2026-05-19：1.37 版。UI 優化。
- 2026-05-18：1.36 版。
  - 新增可自選 LLM 模型。
