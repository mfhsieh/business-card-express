...
- **🕒 歷史紀錄與備份**
  ...
  利用 IndexedDB 於本機保存紀錄（手動修改自動同步），支援 JSON 匯出與匯入，並具備「一鍵瘦身」功能可將原圖轉為縮圖以節省儲存空間。
...
- 2026-08-29：1.47 版。
  - 新增支援 Gemini 3.7 / 3.5 / 3.1 系列模型。
  - 核心 API 現代化（全面改用非同步 Clipboard API、TextEncoder 編碼）。
  - 效能與記憶體優化（歷史紀錄圖片改用 Blob URL、降低自動儲存頻率）。
  - 系統穩定性強化（修正事件監聽洩漏、重試上限防護、CSS 污染隔離）。
- 2026-05-21：1.45 版。
  - 支援同時分享 vCard 與名片圖片。
  - 歷史紀錄升級 UUID 機制，避免匯出入衝突。
  - 介面細節優化（文字大小、視覺漸層、更新圖示）。
- 2026-05-20：1.44 版。
  - 整合 Web Share API，支援手機端一鍵分享名片 (vCard)。
  - 新增歷史紀錄匯出/匯入功能，並支援圖片「一鍵瘦身」機制。
  - 自建本地 SVG 圖示、圖片上傳自動壓縮...等功能，並優化 UI 細節。
- 2026-05-18：1.36 版。
