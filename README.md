...
- **📄 智慧 OCR 與結構化分析**
  - 上傳名片照片，自動辨識並分類至對應欄位（姓名、職稱、公司、電話、Email、地址等）。

- **🔍 深度背景調查**
  - 不僅分析名片字面資訊，AI 還會主動上網搜尋該公司背景與名片主人的專業經歷，並彙整出摘要。

- **✉️ 一鍵產生致謝郵件**
  - 根據名片資訊與背景調查結果，自動草擬專業的拜訪後致謝郵件。

- **📱 掃碼即存 (vCard QR Code)**
  - 自動產生符合 vCard 3.0 標準的 QR Code，用手機一掃即可快速加入通訊錄。

- **📥 vCard 匯出與複製**
  - 支援下載 `.vcf` 檔案或直接複製 vCard 內容，亦強制標註分類為「名片快手」方便您未來整理。
  - 使用手機瀏覽器下載 `.vcf`，會自動觸發手機的「新增聯絡人」機制。

- **🕒 歷史紀錄本機儲存**
  - 自動保存最近辨識的名片紀錄於瀏覽器，方便隨時查閱。
...
3. **直接執行**
   ...
   沒有繁雜的 Node.js 模組安裝與打包。只需在瀏覽器中開啟 `app.html` 即可！
...
4. **透過 Gemini Canvas 快速執行 (免設定 API 金鑰)**
   ...
   - Gemini 會幫您渲染出完整的網頁介面，您可以在右側預覽畫面中直接操作名片快手。
     ...
     > 💡 **提示**
     >
     > 若覺得編輯器中的預覽畫面太小，您可以將該 Canvas 分享 (右上角的分享按鈕)，再透過開啟自己分享的連結網頁，就可以獲得較大的全螢幕操作畫面。
...
4. 在左下角「AI 洞察」區塊查看公司/個人背景摘要，或點擊「後續追蹤」讓 AI 撰寫一封感謝信。
5. 下方會即時產生 QR Code，可以直接用手機相機掃描加入聯絡人，或是使用右下角的「下載 vCard」按鈕將檔案存入電腦、或是在手機上新增聯絡人。
